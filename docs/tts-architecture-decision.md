...
- **CosyVoice2 (0.5B)**：音质上限（**已实现，第二后端，可选项**）。原生流式 + 3s 音色克隆 + 方言，中文第一梯队；代价是 CPU 慢、需 GPU（实测 ~2.9–4.6GB VRAM）、依赖链重（LLM + flow matching + vocoder + CUDA）、首包 ~1.5–1.8s 达不到 <1s、**0.5B LLM 生成长度有随机性**（transformers 修复后已基本解决，见下节 ⚠️）。
...
- **实测（RTX 2070S，时序数字有效）**：加载 ~16s；默认音色 TTFA≈1.5s / 克隆≈1.8s；峰值显存 2.89GB/4.57GB。达不到 melo <1s 硬指标，如实记录——实时主力仍是 melo。⚠️ 接入初期的"验收"其实跑在坏管线上（见下），TTFA/显存与内容正确性无关、仍有效；音质/克隆结论以修复后的试听为准。
