...
### 为什么 melo 快、cosy 慢：非自回归 vs 自回归（2026-08-26）

melo 快不是因为"没有小 GEMV"，而是**根本不做自回归**——VITS 架构一次前向出整句波形；
cosy 的慢是为"音质 + 克隆"付的架构税（LLM 必须逐 token 串行生成）。

| 维度 | MeloTTS（VITS） | CosyVoice2-0.5B（LLM） |
|---|---|---|
| 生成方式 | **非自回归**：文本→波形一次前向 | **自回归**：LLM 逐 token 串行生成 speech token |
| 前向次数 | 1 次（`infer` 内 6 个并行张量算子，无循环） | ~100-400 次（每句，逐 token，依赖上一次输出） |
| 模型规模 | ~200MB（VITS 小模型；BERT 670MB 仅编码端跑一次） | 2.4GB（LLM 1.88G + flow 0.43G + hift 0.08G） |
| 总计算量 | 小模型 × 1 | 0.5B × 每次前向 × ~200 token |
| TTFA（GPU） | ~0.2s | ~5.6s（=整句合成耗时） |
| 代价 | 表达力天花板 + 无音色克隆 | 自回归注定 RTF≈1，需更强卡提速 |

**结论**：melo 的"小 GEMV"总共只跑一遍，cosy 的要跑几百遍且串行依赖盖不住延迟——
差距是**架构取舍**，不是实现优化问题。cosy 的 RTF>1 瓶颈分析见 §6。
