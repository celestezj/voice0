...
> 定位：与 MeloTTS **互补**的第二后端。melotts 只有一种中文女声、无音色克隆；cosy 提供 **中文第一梯队音质 + 任意 3s 参考音频零样本克隆**。代价是首块 ~1.5s（达不到 melo <1s 硬指标）、需 GPU 常驻、且 **0.5B LLM 生成长度有随机性**（见 §8）。实时主力仍是 melo，cosy 是"音质 + 克隆"可选项。
>
...
| 首块 TTFA（实测） | ~0.2s（GPU）/ ~0.9s（CPU） | **~1.5s（默认）/ ~1.8s（克隆）** |
...
```
CosyVoice2 bench: device=cuda（bench/bench_cosy.py --device cuda）
[默认音色] …（加载~16-22s, 峰值显存 2.89GB）
  句1 TTFA=1.509s   音频 0.56s   （首句）
  句2 TTFA=5.454s   音频 16.00s
  句3 TTFA=30.710s  音频 8.80s
  句4 TTFA=41.873s  音频 8.80s
[3s克隆]   （加载~13s, 峰值显存 4.57GB）
  句1 TTFA=1.818s   音频 0.56s
  句2 TTFA=6.037s   音频 16.00s
  …
```

- **模型加载 ~16s**（fp32 纯 torch，jit/trt/vllm 全关），首次推理预热 ~5s。
- **首句 TTFA ~1.5s（默认）/ ~1.8s（克隆）**——达不到 melo <1s，如实记录。
- **峰值显存 2.89GB（默认）/ 4.57GB（克隆）**——2070S 8GB 可跑，克隆略高。
- **注意：音频时长为随机值**（见下），上表是单次采样，不代表稳定指标。
- ⚠️ **本节与接入初期的"验收"是在坏管线上测的**（main env transformers 4.57.6 产出杂音，见文首）。TTFA / 显存 / 加载耗时是**与内容正确性无关**的时序数字，仍然有效；但当时的**"音质更佳 / 克隆音色≈参考音频"结论并未验证**——pin 到 4.51.3 修复后已用 Whisper(base/small/medium) 听写确认内容正确，音质/克隆效果以修复后的试听为准。
